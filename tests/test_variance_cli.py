...
def test_main_prints_clean_error(monkeypatch, capsys, tmp_path):
    monkeypatch.delenv("COHERENCE_DEBUG", raising=False)
    monkeypatch.setattr(
        "sys.argv", ["coherence-variance", "report", "-r", str(tmp_path / "missing")]
    )
    ...
    assert "COHERENCE_DEBUG=1" in err


def test_main_reraises_with_debug_env(monkeypatch, tmp_path):
    monkeypatch.setenv("COHERENCE_DEBUG", "1")
    monkeypatch.setattr(
        "sys.argv", ["coherence-variance", "report", "-r", str(tmp_path / "missing")]
    )
