...
# ceiling).
uv run twominds run --all-questions --n 12 --model-concurrency 4 \
...
uv run twominds analyze  -r results/twominds/run1 -b local -b openai-3-small
uv run twominds report   -r results/twominds/run1
```

## The per-model store — results are reused across runs

`run` / `generate` cache per-model artefacts under `results/twominds/models/`
(`store.py`):
...
uv run twominds analyze -r results/twominds/my_run --judge-run rep3
