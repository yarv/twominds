...
2. ``analyze``   – a cross-sample LLM judge (sees all N responses to one
   question), itself an Inspect ``eval`` logged under ``<run>/judge_logs/``;
   verdicts, per-question answer spread and per-model scores go to
   ``analysis.json``.
3. ``report``    – a self-contained HTML view for eyeballing the variance.

The canonical entry point is the ``twominds.cli`` Typer app, exposed as the
``twominds`` console script (``uv run twominds ...`` from a checkout).
"""
