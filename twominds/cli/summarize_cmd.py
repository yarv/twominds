...
def summarize(
    ...
    """(beta) Add per-model LLM summaries to an existing run, then rebuild the report.

    One judge-model call per model reads its judge verdicts, flags, and sample
    answers and writes a short "what stands out" blurb for the Qualitative tab.
    Beta: summaries are still rough; never run by default (see `run --summaries`).
