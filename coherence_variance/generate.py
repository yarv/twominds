...
named ``response_variance:<rung>``) — Inspect runs them concurrently in one
...
def build_task(questions: list[Question], name: str = "response_variance", model=None):
...
def run_generation(
    ...
    *,
    ...
    model, named ``response_variance:<rung>`` so same-id rungs are tellable apart
    ...
    tasks = [
        build_task(questions, name=f"response_variance:{spec.name}", model=model)
