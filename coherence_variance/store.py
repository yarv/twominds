...
config) — not on which run asked for them. The store keeps them per model:
    ...
    results/variance/models/<model_name>/
...
Run dirs (results/variance/<timestamp>/) stay the aggregation view: their
...
def store_root(results_root: Path | str = "results/variance") -> Path:
